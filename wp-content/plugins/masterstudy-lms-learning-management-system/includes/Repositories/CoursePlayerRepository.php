...
final class CoursePlayerRepository {
	...
	public function get_quiz_data( int $quiz_id, int $user_id = 0 ): array {
		...
		$quiz_data['show_answers'] = ( $this->data['lesson_completed'] ?? false ) || ( ! empty( $quiz_data['last_quiz'] ) && $quiz['correct_answer'] );
		...
		if ( ! empty( $quiz['questions'] ) ) {
			...
			if ( ! empty( $quiz_data['questions'] ) ) {
				...
				if ( ! empty( $quiz_data['last_quiz'] ) ) {
					$sequence = json_decode( $quiz_data['last_quiz']['sequency'], true );
